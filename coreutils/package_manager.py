...
class PackageManager(object):
    ...
    def check_package_path(self, pkg):
        ...
        if check_package_name != 0:
            ...
            print("[root-get] We would use a package/module from {0:s}".format(src_dir_root))
        return src_dir_root
    ...
    def generation_db_modules(self):
        ...
        print("[root-get] DEBUG: Generating DB for modules")
    ...
    def generation_hardcoded_db(self):
        """ We can use either db.hardcoded_db() or db.generated_manifest()."""
        print("[root-get] DEBUG: Generating hc DB for modules")
    ...
    def dag_pre_generation(self, db_manifest):
        ...
        print("[root-get] DEBUG: Generating pre-DAG database")
        dag_manifest = self.database.pre_dag(db_manifest)
        print(json.dumps(dag_manifest, indent=4))
        return dag_manifest
    ...
    def naming_checker(self, pkg):
        print("[root-get] DEBUG: since a name of ROOT package and name of \
directory are not consistent, we need to use it the same as name of target.")
        ...
        if get_close_matches(pkg, target):
            print("[root-get] The package name {0:s} is available in list of  \
    ...
    def check_module_db(self, module, db_manifest):
        print("[root-get] DEBUG: Checking module in DB.")
        try:
            print("[root-get] Checking module DB keys available in root-get: {0:s}".format(db_manifest.keys()))
            if module not in db_manifest.keys():
                print("[root-get] Can't find module {0:s} in DB".format(module))
                return False
        except:
            pass
    ...
    def build_package(self, pkg, db_manifest):
        print("[root-get] DEBUG: Building package: we are using Ninja build system.")
        try:
    ...
    def build_module(self, module, db_manifest):
        print("[root-get] DEBUG:  we are using Ninja build system.")
        try:
            print("[root-get] Installing {0:s}".format(module))
            try:
                print("[root-get] DEBUG: we are running ninja for {0:s}".format(db_manifest[module]["path"]))
            except:
                pass
    ...
    def prepare_module(self, module):
        print("[root-get] DEBUG: Creating Zip file from build directory of module")
        ecpackaging = prepare_module(module)
        if ecpackaging != True:
            print("[root-get] Failed to create module.")
            return False
    ...
    def deploy_module(self, module):
        print("[root-get] DEBUG: Deploying module")
        ecinstall = install_module(module)
        if ecinstall != True:
            print("[root-get] Failed to install module in zip format.")
            ecinstallninja = install_module_ninja(module)
            if ecinstallninja != True:
                print("[root-get] Failed to install module using build system")
                return False
        else:
            return "deployed"
